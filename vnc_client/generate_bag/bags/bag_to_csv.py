...
# Hacky workaround to read bag files from a specific directory for testing

def convert_to_csv(numberOfFiles, listOfBagFiles):
    count = 0
	...
	try:	#else already exists
		...
		pass
        # Hacky workaround used for testing with relative paths
        if "/bags/" in bagName:
		shutil.copyfile(bagName, folder + '/' + bagName.split("/bags/")[1])
	else:
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) > 2):
        ...
        sys.exit(1)
    convert_to_csv(numberOfFiles, listOfBagFiles)
